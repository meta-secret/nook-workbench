...
each applicable merged head. Scheduled, manual, and minds-only PR ecosystem
entry points remain available without starting a second Main-push run. Trusted
PR consumers do not create empty runs for the PR-close event.

## Scope

- Call the reusable Rust ecosystem jobs from `main.yml` in parallel.
- Remove the separate Rust ecosystem `main` push trigger.
