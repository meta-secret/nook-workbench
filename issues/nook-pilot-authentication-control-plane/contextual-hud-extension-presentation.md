...
- 2026-08-25: Exact head `ef61404f43bd31d2ab3c550dfa088099f778d4a7` includes current main `a064a47c68eb9122f9045290ac434d6c2409fc0f`. The prior exact-head Codex review settled clean with zero unresolved conversations. Hosted validation passed repository policy, native Rust, WASM, dependency/RustSec, web verification, WASM Node, coverage, fuzz, Kani, and Dylint. General browser shards failed only in current-main vault, passkey, and preview-URL assertions. The slice-specific headless passkey demo exposed a fixture-only DOM transition that invalidated Pilot presentation; the demo now records activation without manufacturing an authentication-page mutation and passes locally against system Chrome. Exact-head hosted validation and Codex review are restarting.\n\n- 2026-08-25: Four late exact-head P1 review threads were addressed and resolved at `dc91789bbe7a6a1d0585b751f8b2df98eeb7cd87`. Newly exposed QR/recovery evidence now invalidates stale DOM actions before Rust reclassification; explicit collapse is scoped to one presentation; enrollment copy follows the Rust-approved action; and delayed staged TOTP responses recheck authorization and scrub stale code before any page fill. Targeted replies are visible and unresolved review count is zero. The extension passes 259 tests with clean lint, TypeScript, and Svelte diagnostics; the focused passkey-handoff UI demo, architecture, TypeScript-state, Cortex audit, session hygiene, formatting, and pre-push gates pass. Current main remains `a064a47c68eb9122f9045290ac434d6c2409fc0f` and is included. Exact-head hosted validation and Codex review are restarting.\n\n## Findings and decisions
...
- 2026-08-25: Follow-up review fixes reached exact head `629c849c61086092fcdb9e58d02aa0f5da8fa7c2`. Authorization cleanup now closes the session and clears surfaces independently of picker-storage failure; revocation attempts both cleanup paths; routine refresh preserves staged enrollment; confident passkey matches reach Rust without a labeled control and Use can activate the scoped authentication advance while Create still requires an explicit passkey control; the remote container ceiling matches the 45-minute maximum routed timeout. The extension passes 251 tests, lint, TypeScript, Svelte, failure-injection routing tests, remote-execution preflight, and two focused Brave enrollment flows. All review conversations are resolved. Exact-head validation/review were restarted; shared browser-image setup remains unstable and the focused extension run failed before product execution.- 2026-08-25: Exact head `593423261ce9154dabe676e61d66b7d98dd49a06` aligns three hosted extension E2E contracts with the shipped presentation policy: explicit expansion persists across same-workflow remount, a locked actionable page returns as compact Pilot, and passkey picker creation uses runner-safe waits. Root formatting, extension lint, and the pre-push UI-demo contract pass. Current main `a064a47c68eb9122f9045290ac434d6c2409fc0f` remains included; repository policy passed and exact-head PR validation plus Codex review are running. The PR remains draft.
- 2026-08-25: Hosted extension execution on `593423261ce9154dabe676e61d66b7d98dd49a06` passed 33 of 34 tests and proved the two updated presentation expectations. Its sole failure showed the passkey-session fixture still bypassed the intentional compact-first login state. Exact head `bd58bfd675af971c64b77c7dc5bae613024c88a3` now expands the compact Pilot before requesting Fill saved login. Formatting, extension lint, and pre-push checks pass; exact-head validation and Codex review are running. The PR remains draft.

- 2026-08-25: Main a064a47c remains included. Four new P1 review threads were fixed at exact head 09f76fedef6bc4395e5fbe46b78ff7e8f34da5c7: hidden page-wide checkpoints no longer suppress valid login Pilot, backup-code save requires extracted candidates, pre-stage enrollment remounts when QR or recovery evidence changes, and failed toolbar-popup opening falls back to an extension-owned tab. Targeted replies are visible and unresolved review count is zero. Focused extension E2E passed 34/34. Full exact-head validation and Codex review are running; two unchanged focused web/check attempts failed before tests at the shared BuildKit closed-pipe boundary. The PR remains draft.

- 2026-08-25: Exact-head PR run 32909729318 passed native Rust, WASM, Web verification, full extension E2E, headless UI demo, Verify and preview, and Pages deployment. The overall run failed only in current-main local-vault cleanup and canonical crawl assertions outside this slice. Readiness confirms the head is current and mergeable with zero unresolved threads and a successful exact-head deployment; it remains intentionally blocked by draft status, the broad run conclusion, and pending exact-head Codex review.
