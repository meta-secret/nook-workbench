...
One Gizmo Prime decomposes a feature into semantic pull-request slices and
assigns each slice to a uniquely named Gizmo. Each Gizmo coordinates only its
declared Team Agent DAG and returns an exact-head PR handoff. Gizmo Prime accepts
those handoffs, owns the native GitHub stack, and may issue corrected contracts
only to Gizmos that have not started; active work requires a new immutable
generation.
...
- Add a typed Gizmo Prime mission plan with unique Gizmo names, exactly one PR
  slice per Gizmo, dependency order, stable interfaces, a one-level controller
  hierarchy, and explicit Team Agent task ownership.
