...
- Superseding ownership correction: Core must classify raw target-grant storage as absent, invalid, or authorized, binding the grant payload to the requested storage key. Expose a closed typed WASM result; TypeScript only transports observations and executes browser cleanup. Cover missing, malformed, valid exact, mismatched-key, and unrelated-row cases in Rust and the ABI.
- Superseding active-authority correction: classify inside one existing queued offscreen operation using the actual WASM decrypted-vault scope. Missing authority for that active vault must close and clear; unrelated missing authority preserves the installation-wide session. Add a dedicated typed request rather than widening generic Status or observing scope outside the queue. Core owns the active-same/different/none decision; existing cryptographic import validation remains unchanged.
...
- Sequential DEV-CORE and WEB-DEV writers with read-only SECURITY review; Gizmo owns delivery. No local product builds or browser test execution.
...
- Owning modules, packages, or layers: Web test fixtures and helpers; companion Core grant authority and session request contracts; WASM manager active scope; extension queued offscreen handler, background pairing-import and lifecycle routing; focused domain, ABI, adapter, and routing regressions.
- Ownership units:
...
3. Capability: Classify exact target grant authority in Rust; Gizmo ID: restore-e2e-collection; Functional owner: Development core; Expertise provider: None; Expertise allowed code paths: None; Expertise allowed test paths: None; Expertise forbidden paths: None; Expertise consumer interfaces: None; Expertise acceptance evidence: None; Capability acceptance evidence: Behavior-focused Rust tests and typed WASM ABI cases cover missing, malformed, valid, mismatched-key, and unrelated-row storage.
...
1. Gizmo ID: restore-e2e-collection; Gizmo name: Restore authentication E2E; Predecessor Gizmo ID: None; Restore authentication E2E and independent extension session retention; Estimated authored changed lines: 1200; Acceptance evidence: Hosted browser and extension E2E plus required PR checks and exact-head security review pass.
