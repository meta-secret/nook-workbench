---
title: Nook WASM secret-import coverage slice
feature: unplanned
...
pull request 1519. Add a bounded set of behavior-focused tests for the
deterministic secret-import manager boundary, then raise the executable
`nook-wasm` floor only when exact hosted coverage proves a safe increment.

## Requirements

- Exercise deterministic import wrapper, empty-session, and commit-result paths
  without weakening Sentinel, vault, or device-identity gates.
...
- Owning module: `nook-wasm` secret-import manager boundary and colocated
  browser/native tests.
- Delivery shape: One PR
- PR sequence mode: One PR
- Current PR slice and acceptance evidence: Cover deterministic import wrappers,
  duplicate/replacement commit accounting, static gates, exact hosted
  validation, independent coverage, readiness, squash merge, and Workbench
  closeout.

## Initial plan

1. Start from merged Main `861a3bbdce75c2b040ffd5fcc5a73db925493673` at the
   executable `nook-wasm` floor of 78.5 percent.
2. Inspect existing secret-import unit/browser tests and add focused behavior
   tests for uncovered deterministic branches without changing production code.
...
- Tests cover import wrapper and commit-result branches with deterministic
  local fixtures.
