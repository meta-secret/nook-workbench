---
title: Nook WASM password-unlock coverage slice
feature: unplanned
issue: issues/unplanned/rust-crate-coverage-90.md
started_at: 2026-09-07T15:44:56Z
agent: codex
gizmo_id: rust-crate-coverage-90
supersedes: plans/unplanned/20260907T133700Z-nook-wasm-coverage-multi-device.md
---

## Interpreted request

Continue the user-authorized serial coverage mission from merged Main after
pull request 1524. Add a bounded set of behavior-focused tests for the
deterministic `manager/password_unlock.rs` boundary, then raise the executable
`nook-wasm` floor only when exact hosted coverage proves a safe increment.

## Requirements

- Exercise password-unlock success, failure, state, and persistence/error
  branches through the existing local manager fixtures and public boundary.
...
- Owning module: `nook-wasm` password-unlock manager boundary and colocated
  browser tests.
- Delivery shape: One PR
- PR sequence mode: One PR
- Current PR slice and acceptance evidence: Cover deterministic password
  unlock branches, static gates, exact hosted validation, independent
  coverage, readiness, squash merge, and Workbench closeout.

## Initial plan

1. Start from merged Main `164cf8ab611dd75b4ab6b9b17f091387c288571f` at the
   executable `nook-wasm` floor of 79.5 percent.
2. Inspect the password-unlock implementation and existing browser fixtures;
   add focused behavior tests for uncovered deterministic branches without
...
- Tests cover password-unlock behavior through the real WASM manager boundary
  with local deterministic fixtures.
