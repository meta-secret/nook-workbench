...
issue: null
started_at: 2026-09-07T04:45:53Z
agent: codex
gizmo_id: reactive-pr-steward
supersedes: plans/agent-orchestration/20260907T035632Z-reactive-pr-steward-superseding.md
---
...
Steward Team Agents running on a developer machine. Each child must expose
events only for the one pull request assigned by its Gizmo. When a Gizmo moves
to another sequential pull request, it must stop and drain the old child before
starting a new subscription packet. Retain one persistent NATS JetStream
cluster, connect Argo Events to it, and expose only authenticated secure
WebSockets.
...
- Require the active pull-request number at client startup and suppress every
  other pull request before an event can reach PR Steward output.
- Replace a sequential pull-request subscription only through Gizmo's explicit
  stop-old, drain, then start-new lifecycle.
...
- Estimated authored changed lines: 1475
...
- Current PR estimated authored changed lines: 1475
- Current PR slice and acceptance evidence: Deliver the complete single-cluster reactive PR event path; Acceptance evidence: focused infrastructure and Loom checks, live signed GitHub event publication, per-PR suppression proof, authenticated external WSS consumption, explicit replacement shutdown proof, hosted exact-head validation, security review, readiness audit, and remote merge verification
- PR slices, estimates, and acceptance evidence:
1. Gizmo ID: reactive-pr-steward; Gizmo name: Reactive PR Steward; Predecessor Gizmo ID: None; Deliver the complete single-cluster reactive PR event path; Estimated authored changed lines: 1475; Acceptance evidence: focused infrastructure and Loom checks, live signed GitHub event publication, per-PR suppression proof, authenticated external WSS consumption, explicit replacement shutdown proof, hosted exact-head validation, security review, readiness audit, and remote merge verification
...
2. Enable native authenticated WSS, publish the dedicated hostname, provision
   ...
   signed GitHub event.
3. Document and implement the one-PR reactive PR Steward subscription,
   notification, final reconciliation, and stop-old/start-new Gizmo contract.
...
- The client emits events for only its assigned pull request, suppresses a
  simultaneously observed different PR, drains the old subscription, and then
  accepts the next Gizmo-assigned pull request through a fresh child.
