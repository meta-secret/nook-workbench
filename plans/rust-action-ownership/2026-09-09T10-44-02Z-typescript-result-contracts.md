---
title: Use explicit TypeScript Result failure contracts
...
Prohibit authored TypeScript exception-based failure paths and use explicit typed Result success/failure contracts. Define a minimal shared convention or established library, migrate domain/application callers and asynchronous returns, and translate unavoidable host/library exceptions at narrow boundaries. Preserve domain-specific failure vocabulary and require caller handling. The user explicitly authorizes a concrete generic Result type, superseding earlier local prohibition against repository-defined generic results.
...
1. Restore the mandatory delegation prerequisite blocked by the prior runtime dependency declaration, subject to the required gate exception.
2. Choose one minimal typed Result convention and update Cortex, removing the conflicting generic-result prohibition for this explicit contract.
3. Inventory and migrate authored TypeScript throws/rejections and callers by owner; catch external exceptions at boundaries.
4. Publish scoped commits and an accurate PR handoff, with verification deferred.
