---
title: Fix all PR feedback before broader API migrations
feature: rust-action-ownership
issue: issues/rust-action-ownership/global-type-safety-refactor.md
started_at: 2026-09-09T10:59:48Z
agent: codex
...
Prioritize the latest explicit request: address all actionable PR1573 comments and push. Preserve the pending broader Result, static-method and consuming-state migrations as queued scope. Finish the already-started coherent policy/dependency/instance-builder foundation without starting incompatible Result APIs, then apply Rust and browser review fixes. Tests and compilation remain deferred; author requested regression coverage without running it.
...
1. Complete the prerequisite gate/lock/library import repairs and the currently coherent policy/dependency foundation.
2. Fix seven Rust findings with direct regression sources, including boolean wire compatibility, tolerant heterogeneous imports, translation nodes, coherent activity state and control transport coverage.
3. Fix live browser findings and password-wire callers; confirm two outdated idle findings from current source and add requested regression coverage.
4. Push, then reply and resolve addressed threads with exact commit evidence and unexecuted-test disclosure.
5. Publish completed comment scope and explicitly queue broader Result/static/consuming-state migrations rather than claiming they are done. No tests or merge.
...
This plan supersedes the preceding combined feedback/pending-contract plan for sequencing. Latest explicit comment-fix delivery is prioritized; broader migration inventories remain open for the subsequent pass. Twelve threads were collected, including two outdated but unresolved threads requiring source disposition.
