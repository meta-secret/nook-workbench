...
Eliminate helpers that take a whole authored object solely to inspect its enclosed kind or value. Place kind-only classification on the semantic kind owner and expose meaningful aggregate instance methods that delegate when callers operate on the aggregate. Apply the same receiver-placement rule to static helpers, nested-value interpretation and concrete equivalent smells across tooling, Rust, web and infrastructure. Update canonical Cortex guidance with language-appropriate examples and preserve generated/wire vocabulary.
...
1. Discover concrete overwide object-parameter helpers under each owner through source inspection.
