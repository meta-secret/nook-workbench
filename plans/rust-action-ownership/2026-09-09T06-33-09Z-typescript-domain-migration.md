...
Extend PR #1573 by identifying TypeScript boilerplate and misplaced portable product logic. Move business decisions, validation, policy and security behavior that belongs in Rust into cohesive Rust owners with typed WASM interfaces. Remove duplicated TypeScript implementations and simplify affected consumers. Retain browser effects, presentation and intentional TypeScript repository tooling in their appropriate layer.
...
Development core owns product Rust/WASM in nook-app/nook-platform. Web development owns browser TypeScript/Svelte. AI and SRE inspect their TypeScript for misplaced product logic while preserving legitimate repository/host tooling. Each owner records concrete candidates and retained boundaries. Product validation and policy move into existing appropriate Rust crates; browser code consumes typed outputs. Preserve existing typestate, serialization, cryptography, authorization, storage and secret lifetime contracts. No generic abstraction framework or mechanical language conversion.

## Initial plan

1. Inventory concrete TypeScript domain-placement candidates and identify existing Rust owners and typed boundary requirements.
2. Implement applicable Rust owners and WASM contracts with coherent source examples.
3. Remove duplicate TypeScript decisions and adapt consumers with one writer at a time.
4. Resolve discovered cross-team dependencies and record retained host/presentation/tooling boundaries.
5. Commit and push completed changes, update PR #1573 and Workbench records; all verification awaits separate user instruction.
