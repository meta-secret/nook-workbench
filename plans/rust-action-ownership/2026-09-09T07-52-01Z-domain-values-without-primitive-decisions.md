...
Refactor domain decisions that unwrap model values into primitive tuples or compare unexplained raw constants. Start with authentication snapshot step-contract matching and extend to concrete analogous product/tooling/browser cases. Keep legal domain combinations, progress and validation on meaningful model owners. Preserve primitive representation only for actual encoding, arithmetic, indexing and host boundaries; do not mechanically wrap every literal.
...
Development core owns authored product Rust/WASM domain models and snapshot contracts. SRE owns applicable Hive/preflight domain decisions. Web owns affected browser consumers. Each team inventories concrete domain leaks and distinguishes legitimate primitive representation boundaries under existing Cortex typed-newtype/domain API/SRP rules. Writers run sequentially; preserve wire/storage, authorization, cryptography and runtime freshness. No policy change is necessary unless discovery finds an actual gap in the existing rule.

## Initial plan

1. Trace raw-value decisions and identify their model owners and canonical domain states.
2. Refactor applicable Rust domain contracts and direct callers/source examples.
3. Migrate affected browser and tooling consumers, removing duplicated primitive decisions.
4. Record retained primitive boundaries and remove superseded APIs.
5. Commit and push to PR #1573, publish handoff with all verification deferred.
