...
Extend PR #1573 by replacing loosely structured JSON and JavaScript values with named serde structs, enums, and typed browser contracts throughout the repository. Keep serialization at actual transport boundaries, adopt serde_json::Result where appropriate, and preserve supported wire formats and security behavior.
...
- Estimated authored changed lines: 120000
...
- Current PR estimated authored changed lines: 120000
...
## Typed serialization scope

Development core owns nook-app/nook-platform Rust serialization and WASM contracts. Web development owns nook-app/nook-web consumers. AI owns Loom and authored Cortex executable scripts plus a durable rule clarification. SRE owns preflight, Hive, CI and infrastructure tooling. Each stage inventories remaining dynamic JSON uses and documents boundary exceptions. No tests, compilation, reviews, or remote validation workflows will run. Existing PR #1573 remains the delivery target.
