...
started_at: 2026-09-04T21:50:00Z
agent: codex
...
- The checked capability is an opaque public type with private fields; no unchecked construction, Clone, Copy, or Deserialize.
- Keep raw request and response DTO wire forms unchanged.
- Move the small core signing adapters onto the core-owned SentinelUnlockSigning extension trait implemented for SigningIdentity and migrate every direct caller.
...
- This supersedes the initial plan to clarify that SigningIdentity belongs to event-log; a core extension trait avoids illegal foreign inherent methods and dependency cycles.
...
- Public or cross-module interfaces: Auth2 raw request transitions to checked request and encrypted response; core SentinelUnlockSigning extends the event-log-owned SigningIdentity with signing adapters; WASM export and wire contracts remain stable
