---
title: Use library schema decoding for article transport
...
Replace CortexArticleTransport's handwritten structural JSON decoding with an established schema-validation library. JSON syntax already uses the native parser; remove repetitive record, key, field and array decoding while retaining canonical domain invariants, bounded input/output, stable failures and existing wire semantics. Scope the article codec and its directly shared contract/dependency consumers rather than unrelated transports.
...
AI owns the article codec, domain/schema declarations, direct action schema consumers and required package/dependency lock entries. Choose an established library from actual dependency availability; do not author another schema engine. Preserve strict keys, bounds, UTF-16 versus UTF-8 semantics, uniqueness/order/capacity, error categories/paths and sensitive-input handling. Inferred schema types should replace duplicate structural DTO declarations where safe. Do not remove genuine cross-field domain admission.

## Initial plan

1. Inspect the codec and existing schema/library ownership.
2. Select an established validator and publish exact dependency scope.
3. Replace structural decoding with schema admission and adapt direct callers/source fixtures.
4. Commit, push and update PR #1573 with the unexecuted verification limitation.
