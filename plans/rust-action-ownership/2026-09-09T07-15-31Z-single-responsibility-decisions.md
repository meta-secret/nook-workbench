...
Make single-responsibility ownership explicit in Cortex and refactor authentication workflow selection so its result type owns replacement and state interpretation. Callers should orchestrate through intention-revealing operations rather than reconstruct another type's domain rules. Apply the same correction to concrete related cases while preserving selection priority, rejection and tie behavior.
...
AI owns focused canonical Cortex function-ownership policy updates. Development core owns authentication workflow match/selection and necessary direct callers/source examples in nook-companion-core. Writers run sequentially. No new generic framework, decorative wrappers, mechanical getter extraction or unrelated rewrite. Genuine rendering and boundary discrimination remain at those owners.

## Initial plan

1. Identify the precise domain knowledge and related misplaced decisions.
2. Strengthen existing canonical Cortex SRP requirements without duplicating policy.
3. Move decisions onto their meaningful types and simplify callers.
4. Commit and push to PR #1573; publish outcome with all verification deferred.
